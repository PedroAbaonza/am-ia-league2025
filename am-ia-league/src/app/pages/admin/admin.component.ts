...
import { AdminService, AdminConfig, SystemMetrics, AuditLog } from '../../services/admin.service';
...
export class AdminComponent implements OnInit, OnDestroy {
  ...
  private async validateFile(file: File, type: 'squad' | 'individual'): Promise<boolean> {
    ...
    try {
      ...
      if (file.size > 5 * 1024 * 1024) {
        this.fileValidationErrors.push('El archivo es demasiado grande (máximo 5MB)');
      }
      ...
      if (!allowedTypes.includes(fileExtension)) {
        this.fileValidationErrors.push('Tipo de archivo no válido. Solo se permiten archivos CSV');
      }
      ...
      if (this.fileValidationErrors.length > 0) {
        this.showError(`Errores de validación: ${this.fileValidationErrors.join(', ')}`);
        return false;
      }
  ...
  private validateCSVContent(content: string, type: 'squad' | 'individual') {
    try {
      ...
      const requiredFields = type === 'squad' 
      ...
      if (missingFields.length > 0) {
        this.showError(`Campos requeridos faltantes: ${missingFields.join(', ')}`);
        return;
      }
      
      this.showSuccess(`Archivo validado correctamente. ${csvData.length} registros encontrados.`);
    } catch (error) {
  ...
  processSquadFile() {
    ...
    try {
      ...
      this.adminService.logAction('upload_squads', `${squads.length} squads procesados desde ${this.squadFileName}`);
      ...
      this.adminService.logAction('upload_error', `Error procesando squads: ${error}`);
  ...
  processIndividualFile() {
    ...
    try {
      ...
      this.adminService.logAction('upload_individuals', `${individuals.length} desarrolladores procesados desde ${this.individualFileName}`);
      ...
      this.adminService.logAction('upload_error', `Error procesando individuales: ${error}`);
      this.showError('Error procesando el archivo: ' + error);
    }
  }
  ...
  clearAuditLogs() {
    if (confirm('¿Estás seguro de que quieres limpiar todos los logs de auditoría?')) {
  ...
  restoreFromBackup(type: 'squads' | 'individuals') {
    if (confirm(`¿Estás seguro de que quieres restaurar los datos de ${type} desde el backup?`)) {
      try {
  ...
  validateDataIntegrity() {
    ...
    try {
      ...
      if (result.issues.length > 0) {
        this.showError(`Problemas de integridad encontrados: ${result.issues.join(', ')}`);
      } else {
  ...
  toggleIndividualsDetails() {
    ...
    if (this.showIndividualsDetails) {
      this.adminService.logAction('view_individuals', 'Detalles de individuales visualizados');
    }
  }
